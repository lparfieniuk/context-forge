...
NEVER ship a flag, hook, or gate without a negative test that FAILS when the mechanism is removed — an untested control is documentation, not enforcement (`activation: intelligent` was honoured by nothing for months; `enforce-tier-routing.sh` intercepted zero calls in a 30-day window).
NEVER assume `npm run convert` prunes — it writes installed copies and NEVER deletes them, so a removed or demoted module keeps running and keeps billing from its installed path. ALWAYS re-check the installed directory after deleting or demoting a module.
...
| `harnesses/<id>.yaml` | Harness capability profile (what a harness can do; consumed by emitters) |
| `dist/AGENTS.md` | Emitted AGENTS.md-family distillation (auto-generated, gitignored — embeds absolute paths) |

`convert.ts` DOES generate `.claude/rules/<id>.md` (one file per rule with `installed_claude` set — 18 today) and `.cursor/rules/*.mdc`. It does NOT generate `.cursor/skills/`; do not present that path as an active runtime artifact. `emit-agents-md.ts` (`npm run emit:agents`) generates `dist/AGENTS.md` from a harness profile; it is the only emitter for the AGENTS.md family.

Harness neutrality contract: sources in `core/skills/` and `core/agents/` reference the plugin root ONLY as `<CF_PLUGIN_ROOT>` — convert resolves it to `${CLAUDE_PLUGIN_ROOT}` for Claude Code, the emitter to the absolute repo path for AGENTS.md. `audit-portability.sh` enforces this in every plugin-audit run. Rules numbered 800–899 are LOCAL-ONLY: gitignored, never registered in `_index.yaml`, never installed.
