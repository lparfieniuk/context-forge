...
Wraps `bash <CF_PLUGIN_ROOT>/core/scripts/tools/failure-replay.sh --ledger <path>`. Reads a rule-004 failure ledger, classifies whether it is a reproducible code scenario (build/test/tooling signal in goal, tool, or error_trajectory), and for bench-replayable failures scaffolds a DRAFT case into cf-bench:
...
```bash
bash <CF_PLUGIN_ROOT>/core/scripts/tools/failure-replay.sh --ledger <IDE_DIR>/lessons/YYYY/MM/DD/<prefix>/<uuid>-<slug>.yaml
```
