...
Incident-to-regression: a ledger describing a build/test/code failure can be
scaffolded into a DRAFT bench case via `core/scripts/tools/failure-replay.sh`
(`failure-replay` skill) — the recorded incident becomes a replayable
regression case instead of prose. Drafts are inert until a human authors the
fixture and check.sh; process-only failures stay out of the bench by design.
