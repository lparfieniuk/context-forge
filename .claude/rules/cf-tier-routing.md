...
NEVER spawn Task() when a Tier 0 or Tier 1 alternative exists.
...
| 2 | `Task(model: "haiku")` — Haiku | `claude-haiku-4-5-20251001` | ~200–1k | Multi-file reasoning, LLM judgment |
| 3 | `Task(model: "sonnet")` — Sonnet | `claude-sonnet-5` | ~600–5k | Architecture, planning, complex reasoning |
| 3+ | `Task(model: "opus")` — Opus | `claude-opus-4-8` | ~1k–10k | Novel architecture decisions only |

ALWAYS pick the cheapest tier that produces a correct result.

Task tool model enum: `sonnet | opus | haiku | fable`. `Task(model: "fable")` (Fable 5) is reserved for the hardest, long-horizon work only — it is the most expensive model and sits outside the normal tier hierarchy.
...
ALWAYS default to Haiku (`claude-haiku-4-5-20251001`) for Tier 2.
...
NEVER escalate to Opus (`claude-opus-4-8`) unless ALL of the following are true:
...
Spawn Scribe (`model: "haiku"`) BEFORE adding any large output to primary context.
Sub-agent responses MUST cap at ≤2,000 tokens — excess tokens cause systematic errors, not just cost.

| Trigger | Threshold | Scribe Task |
|---------|-----------|-------------|
| Bash output (build/test/lint) | >100 lines or >5 KB | Return `[RCA]` — root cause + file + 1-line fix |
| Search results | >50 matches or >5 KB | Return file:line pairs (max 20) |
| Tool results (file reads, search) | >3 KB | Return summary only (lower threshold than general) |
| Worklog read | >15 entries | Return history_summary + last 5 |
| Manifest read | Symbol lookup | Return matching entry only |

Scribe I/O contract: `[SCRIBE OUTPUT] Summary / Key finding: path:line / Recommendation [END SCRIBE OUTPUT]`. Primary agent MUST discard raw input after Scribe output.
...
## Validation gate (MANDATORY before Task() or sub-agent spawn)
...
- [ ] KERNEL template (Context/Task/Constraints/Format/Verify) used in prompt?
