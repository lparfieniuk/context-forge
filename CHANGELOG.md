...
- **Skill telemetry, and the first honest count of what this plugin is used for.** `core/scripts/tools/skill-usage.sh` (Tier 1, no LLM) counts Skill-tool, slash, and sub-agent invocations per skill from Claude Code transcripts; a `PostToolUse`/`Skill` hook (`hooks/skill-usage-log.sh`) appends each invocation to `~/worklogs/logs/skill-usage.tsv` because transcripts are deleted after `cleanupPeriodDays` (default 30, verified in the 2.1.261 binary) — the scan can only ever see a rolling month. Claude Code exports no per-skill OTel metric (`skill_invoke` is internal analytics), so there was nothing to reuse. Result, recorded as H6 in `core/benchmarks/HYPOTHESES.md`: 7 of 23 skills ran at all, 5 ran outside this repo, and zero discovery skills, zero Tier 1 scripts and zero `cf-scribe` spawns happened outside it.
- **Negative scope is now a scored dimension.** Every skill description was rewritten as a routing predicate — literal trigger phrases plus an explicit `Do NOT use for` clause naming the skill that covers the adjacent case — and `quality-score.sh` scores that as its own dimension (skill denominator 6 → 7), scanning the whole frontmatter block so a folded `description: >` is read correctly.
- **Orphan-hook gate.** `convert.ts` writes but never prunes, so a deleted hook keeps running from the installed copy; `scripts/hooks.test.ts` now fails when the marketplace holds a hook file the source has dropped. Both `wiki-nudge.sh` and `pre-compact-anchor.sh` survived their own deletion this way while every other gate passed.

### Changed

- **The compaction anchor moved to the event that can help it.** `pre-compact-anchor.sh` → `compact-anchor.sh`, rebound from `PreCompact` to `PostCompact`: on `PreCompact` it emitted the anchor into the very context compaction then summarised away.
- **`if:` filter on the `grep` hook.** `enforce-rg.sh` carries `"if": "Bash(grep *)"`, so it no longer spawns a shell on every Bash call. The field is real, not assumed: the 2.1.261 binary's command-hook schema reads `type:k("command"),command:s(),args:v(s()).optional(),if:rt(),shell:…,timeout:…`, and `code.claude.com/docs/en/hooks` documents it with a Bash-matching table. It is evaluated ONLY on tool events (`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `PermissionDenied`) — a hook carrying `if` on any other event never runs at all. Permission-rule matching checks each pipeline subcommand and each `$()`/backtick body, which is exactly the hook's own matcher, so no case is lost — verified against four payloads. `pre-commit-review.sh` deliberately keeps no `if`: it also blocks `bash -c "git commit"`, which a command-name pattern would not reach.
- **Sub-agent tool renamed throughout: `Task(...)` → `Agent(...)`** (fields `subagent_type`, `model`, `prompt`), with `Task` documented as the legacy alias the binary still accepts. The `PreToolUse` matcher widened to `Agent|Task` so a harness emitting either name still hits `enforce-tier-routing.sh`.
- **Model ids and prices re-verified 2026-09-05.** Tier 3+ is `claude-opus-5`; Haiku ids lost their date suffix. Rule 013/011 record that Opus 5 has adaptive thinking **ON** by default (the reverse of 4.7/4.8) and that a carried-over `{type: "disabled"}` is a 400 at `xhigh`/`max`. Rule 018's "Sonnet 5 rises to $3/$15 from 2026-09-01" row was **false** — the live pricing page shows $2/$10 still in force; it was Sonnet 4.6's price in a Sonnet 5 row. Added `claude-opus-5` and `claude-fable-5-1` (cache reads at $0.25/MTok, a quarter of Fable 5).

### Removed

- **`task-init`, `update-worklog` and their scripts** (`task-init.sh`, `worklog.sh`) — the Jira ticket track, with no active ticket project and zero invocations.
- **`help`** — the harness injects the full skill listing every session; the skill duplicated it and was never called.
- **`wiki-nudge.sh`** — the backstop fired on `pack-context.sh` Bash calls, of which a 30-day scan found zero. A backstop that cannot fire is not one; `compile-wiki`'s own description carries the trigger now.
- **The `Scribe Dispatch` table in rule 003** shrank to three lines: it described, in every always-on session, a mechanism with zero spawns. Tier 1 `compress-observation.sh` is the default; a Scribe spawn is now the exception that needs a reason.
