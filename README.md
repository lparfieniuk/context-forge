...
```
  ...
  harnesses/<id>.yaml ──npm run emit:agents──▶ AGENTS.md family
                                    (opencode, Codex CLI, Copilot,
                                     Gemini CLI, Aider, Cline/Roo)                .cursor/agents/
...
### Other harnesses (AGENTS.md family)

Sources are harness-neutral: skills and agents reference the plugin root only as `<CF_PLUGIN_ROOT>`,
and each harness gets a capability profile under `harnesses/`. To use ContextForge outside
Claude Code/Cursor:

```bash
npm run emit:agents                                        # → dist/AGENTS.md (gitignored)
ln -sf "$(pwd)/dist/AGENTS.md" /path/to/project/AGENTS.md  # consume per project
```

The emitted file carries the always-on rules verbatim, an index of on-demand rules, the script
catalog with absolute paths, and a harness-adaptation section (no hooks → enforcement degrades to
compliance; tier model ids must be filled in `harnesses/opencode.yaml`). `audit-portability.sh`
gates neutrality on every plugin-audit run.
...
| `core/scripts/tools/*.sh` | Tier 0/1 shell tooling (32 scripts) | **yes** |
| `hooks/*.sh`, `hooks/hooks.json` | Enforcement hooks | **yes** |
| `scripts/*.ts` | Convert pipeline, AGENTS.md emitter, manifest generator, test suite | **yes** |
| `harnesses/<id>.yaml` | Harness capability profiles | **yes** |
| `docs/adr/` | Architecture decision records | **yes** |
| `skills/`, `agents/`, `.claude/rules/`, `.cursor/` | Generated by convert | **never** |
| `dist/AGENTS.md` | Emitted by emit:agents (embeds absolute paths) | **never** |
